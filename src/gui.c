...
static LRESULT CALLBACK wndproc(HWND w, UINT m, WPARAM wp, LPARAM lp) {
    ...
    case WM_CREATE:
        ...
        hDest = CreateWindowExA(WS_EX_CLIENTEDGE, "EDIT", "download", WS_CHILD|WS_VISIBLE|ES_AUTOHSCROLL, 85, 55, 490, 26, w, (HMENU)IDC_DEST, gInst, 0);
        CreateWindowA("BUTTON", "Start", WS_CHILD|WS_VISIBLE, 85, 98, 90, 30, w, (HMENU)IDC_START, gInst, 0);
        CreateWindowA("BUTTON", "Pause", WS_CHILD|WS_VISIBLE, 185, 98, 90, 30, w, (HMENU)IDC_PAUSE, gInst, 0);
    ...
    case WM_COMMAND:
        ...
        case IDC_START: set_status("Downloader engine integration pending: select a .torrent and start the engine."); break;
        case IDC_PAUSE: set_status("Paused"); break;
        }
        break;
...
int gui_run(int argc, char **argv) {
    ...
    WNDCLASSA wc = {0}; wc.lpfnWndProc = wndproc; wc.hInstance = gInst; wc.lpszClassName = "PyTorrentCWindow";
    wc.hCursor = LoadCursor(NULL, IDC_ARROW); wc.hbrBackground = (HBRUSH)(COLOR_WINDOW+1);
    RegisterClassA(&wc);
    HWND w = CreateWindowA("PyTorrentCWindow", "BitTorrent Client", WS_OVERLAPPEDWINDOW & ~WS_MAXIMIZEBOX,
